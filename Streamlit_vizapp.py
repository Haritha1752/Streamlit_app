...
st.title('NYC Job Postings Dashboard')
...
with col2:
    # Visualization 2: Number of Job Postings by Agency
    st.subheader("Number of Job Postings by Agency")
    top_n_agency = df_filtered['Agency'].value_counts().head(10)
    fig_agency = px.bar(top_n_agency, orientation='h',
                        title="<b>Top 10 Agencies by Number of Postings</b>",
                        color_discrete_sequence=px.colors.qualitative.Set2,
                        template="plotly_white")

    fig_agency.update_layout(
        xaxis_title="<b>Number of Postings</b>",
        yaxis_title="<b>Agency</b>",
        title_font_size=20,
        font=dict(
    ...
    fig_agency.update_traces(marker=dict(line=dict(width=1,
                                                    color='DarkSlateGrey')),
                             opacity=0.8)

    # Customizing the hover template to make it more informative
    fig_agency.update_traces(hovertemplate='<b>Agency:</b> %{y}<br><b>Postings:</b> %{x}<extra></extra>')
